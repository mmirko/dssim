...
int check_end(int *states, int *messages, int nodes, int *ending_states, int *ending_messages, int registers, int messtypes)
{
	...
	for (j = 0; j < registers; j++)
	{
		for (i = 0; i < nodes; i++)
		{
			if (*(states + i * registers + j) != *(ending_states + j))
				return 0;
		}
	}
	for (i = 0; i < nodes; i++)
	{
		for (j = 0; j < nodes; j++)
		{
			for (k = 0; k < messtypes; k++)
			{
				if (*(messages + (i * nodes + j) * messtypes + k) != *(ending_messages + k))
					return 0;
			}
		}
	}
...
void default_layout(gdImagePtr *tedim, int tedimx, int tedimy)
{
	...
	if (border > (bordper * tedimy) / 100)
		border = (bordper * tedimy) / 100;
...
int main(int argc, char *argv[])
{
	...
	if (graph_file != NULL)
	{
		...
		{
			if (verbose)
				printf(" - Importing node - %s (Host %p)", agnameof(inode), inode);
			*(ithnode + i) = inode;
		...
		if (verbose)
			printf("%d nodes imported\n\n", nodes);

		if (verbose)
			printf("Importing graph edges:\n");

		for (inode = agfstnode(dsgraph); inode != NULL; inode = agnxtnode(dsgraph, inode))
		{
			if (verbose)
				printf(" - Importing node %s edges:\n", agnameof(inode));
			for (iedge = agfstout(dsgraph, inode); iedge != NULL; iedge = agnxtout(dsgraph, iedge))
			{
				...
				if (verbose)
					printf("   - Importing edge (Label %d) - %s (Host %p) -> %s (Host %p)", *(links + j * nodes + k), agnameof(agtail(iedge)), agtail(iedge), agnameof(aghead(iedge)), aghead(iedge));
	...
	else if ((protocol_file == NULL) && (kernel_file != NULL))
	{
		...
		*(entity + strlen(entityfile) - 3) = 0;
		...
		if (verbose)
			printf("Loading custom OpenCL kernel as protocol:\n   Protocol file: %s\n   Entity OpenCL function: %s\n\n", entityfile, entity);
	...
	{
		if (verbose)
			printf("Ideal time complexity active, speeds will be ignored.\n");
		speed_lcm = 1;
		speed_gcd = 1;
		for (i = 0; i < nodes; i++)
		{
	...
	for (timetick = 1; timetick < sim_time; timetick++)
	{
			...
			for (j = 0; j < nodes; j++)
			{
				...
				for (k = 0; k < messtypes; k++)
				{
					if (*(messages + (i * nodes + j) * messtypes + k) != *(message_defaults + k))
					{
						...
						{
							if (verbose)
								printf("( %s -> %s ) ", tempstr, agnameof(*(ithnode + j)));
							free(tempstr);
						}
						messcompl++;
						messchck = 1;
		...
		if (bypass_ending == 0)
		{
			if (check_end(states, messages, nodes, ending_states, ending_messages, registers, messtypes) == 1)
				break;
		}
	...
	if (lua_check_report(L, "TIME_COMPLEXITY") == 1)
		printf("Time Complexity: %d\n", timetick);
	if (lua_check_report(L, "MESSAGE_COMPLEXITY") == 1)
